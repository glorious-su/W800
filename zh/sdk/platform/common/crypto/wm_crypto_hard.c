...
#define KEY_LEN 16
#define CHAR_LEN 16
#define MAX_CHAR_LEN   65536
int test_rc4(unsigned char* key, int keylen, unsigned char* pt, int len, uint8_t mode){
	int ret = -1;
	//int i = 0, j = 0;
...
out:
	if(ct)
		tls_mem_free(ct);
	printf("test_rc4   ret= %d\n", ret);
	return ret;
}

int test_aes_ctr(unsigned char* key, int keylen, unsigned char * IV, unsigned char* pt, int len, uint8_t mode){
	int ret = -1;
	psCipherContext_t  ctx;
	unsigned char *ct = NULL;
...
int test_aes_cbc(unsigned char* key, int keylen, unsigned char * IV, unsigned char* pt, int len, uint8_t mode){
	int ret = -1;
	psCipherContext_t  ctx;
	unsigned char *ct = NULL;
...
int test_aes_ecb(unsigned char* key, int keylen, unsigned char * IV, unsigned char* pt, int len, uint8_t mode){
	int ret = -1;
	psCipherContext_t  ctx;
	unsigned char *ct = NULL;
...
int test_3des_cbc(unsigned char* key, int keylen, unsigned char * IV, unsigned char* pt, int len, uint8_t mode){
	int ret=-1;
	psCipherContext_t  ctx;
	unsigned char *ct = NULL;
...
int test_3des_ecb(unsigned char* key, int keylen, unsigned char * IV, unsigned char* pt, int len, uint8_t mode){
	int ret=-1;
	psCipherContext_t  ctx;
	unsigned char *ct = NULL;
...
int test_des_ecb(unsigned char* key, int keylen, unsigned char * IV, unsigned char* pt, int len, uint8_t mode){
	int ret=-1;
	psCipherContext_t  ctx;
	unsigned char *ct = NULL;
...
int test_des_cbc(unsigned char* key, int keylen, unsigned char * IV, unsigned char* pt, int len, uint8_t mode){
	int ret=-1;
	psCipherContext_t  ctx;
	unsigned char *ct = NULL;
...
int test_sha1(unsigned char *pt, int len){
	...
	ct = tls_mem_alloc(SHA1_HASH_SIZE);
	if(ct == NULL){
		goto out;
	}
	cth = tls_mem_alloc(SHA1_HASH_SIZE);
	if(cth == NULL){
		goto out;
	}
	psSha1Init(&ctx);
...
//	dumpBuffer("pt", pt, len);
//	dumpBuffer("ct", ct, SHA1_HASH_SIZE);
//	dumpBuffer("cth", cth, SHA1_HASH_SIZE);
	if(memcmp(ct, cth, SHA1_HASH_SIZE) != 0)
		goto out;
	ret = 0;
out:
	if(ct)
...
//	dumpBuffer("pt", pt, len);
//	dumpBuffer("ct", ct, MD5_HASH_SIZE);
//	dumpBuffer("cth", cth, MD5_HASH_SIZE);
...
int test_crc(u32 key, unsigned char *pt, int len, CRYPTO_CRC_TYPE crc_type, uint8_t mode){
	int ret = -1;
...
//	tls_crc_update(&ctx, pt, len);
	...
	{
		TLS_DBGPRT_INFO("crc_result 0x%x, crc_result1 0x%x\n", crc_result, crc_result1);
		goto out;
	}
	ret = 0;
out:
...
{
	u32 val = 0;
	int rmain = len % sizeof(u32);
	rsaRnd(buf, len/sizeof(u32));
	if(rmain)
	{
		val = rand();
		memcpy(buf+len/sizeof(u32), (char *)&val, rmain);
	}
	return 0;
}
#endif
...
int montExptMod(pstm_int *a, pstm_int *e, pstm_int *n, pstm_int *res);

int initPstmParams(u32 len, pstm_int	*pa, pstm_int *pb, pstm_int	*pm, int isRand){
...
int startExpMod(u32 len){
	...
	int err = -1;
	//int i;
	u32 g_datas[4] = {0xa92a9807, 0x1eeddf45, 0xae5327cd, 0x96e0f48f};

	pstm_init(NULL, &pres);

	tls_reg_write32(RSA_BASE_ADDRESS, pres.dp);
	if((err = initPstmParams(len, &pa, &pb, &pm, 0)))
	{
		return err;
	}

	tls_crypto_exptmod(&pa, &pb, &pm, &pres);
//	pstm_exptmod(NULL, &pa, &pb, &pm, &pres);

	if(len != 4 || pres.dp[0] != g_datas[0] || pres.dp[1] != g_datas[1] || pres.dp[2] != g_datas[2] || pres.dp[3] != g_datas[3])
	{
		tls_reg_write32(0x70000000, 0x1);
	}
	tls_reg_write32(0x70001800, 0x1);
	tls_reg_write32(HR_PMU_TIMER0, 0x10000);
#if 0
	printf("pres:\n");
	for(i=0;i<pres.used;i++)
	{
	...
	pstm_clear(&pm);
	pstm_clear(&pres);
//	printf("startExpMod err %d\n", err);
	return err;
}
int rsa_simulation_main(void)
{
	int err;
	unsigned int sec_cfg, val;
	tls_crypto_init();
	...
	val = tls_reg_read32(HR_CRYPTO_SEC_CFG);
	sec_cfg = val | (1 << SOFT_RESET_AES);
	tls_reg_write32(HR_CRYPTO_SEC_CFG, sec_cfg);
	val = tls_reg_read32(HR_CRYPTO_SEC_CFG);

	err = startExpMod(4);
	printf("startExpMod ret %d\n", err);
	return err;
}

static int setRsaParams(pstm_int *a, pstm_int *e, pstm_int *n)
{
//	int i = 0;
	u32 k = 0, mc = 0, dp0;
//	uint8_t monmulFlag = 0;
	pstm_int R, X, Y;
	pstm_init(NULL, &X);
	pstm_init(NULL, &Y);
	pstm_init(NULL, &R);
...
//	u32 mc = 0;
	a = malloc(len * sizeof(u32));
	if(a == NULL)
		goto out;
	b = malloc(len * sizeof(u32));
	if(b== NULL)
		goto out;
	m = malloc(len * sizeof(u32));
	if(m == NULL)
		goto out;
#if 1
	rsaRnd(a, len);
...
/*     res = a * b * 2^(-k) % n        */
int montMulMod(pstm_int *a, pstm_int *b, pstm_int *n, pstm_int *res)
{
	int i = 0;
	pstm_int tmp;
	u32 len = n->used * DIGIT_BIT;//pstm_count_bits(n);
	pstm_init(NULL, &tmp);
	pstm_zero(&tmp);//C' = 0
	for(i=0;i<(len);i++){
		if(pstm_get_bit(a, i))
			pstm_add(b, &tmp, &tmp);
		if(pstm_get_bit(&tmp, 0))
			pstm_add(n, &tmp, &tmp);
		pstm_div_2(&tmp, &tmp);
		if(pstm_cmp(n, &tmp) != PSTM_GT)
			pstm_sub(&tmp, n, &tmp);
	}
	pstm_copy(&tmp, res);
	pstm_clamp(res);
...
int montExptMod(pstm_int *a, pstm_int *e, pstm_int *n, pstm_int *res)
{
	...
	pstm_init(NULL, &Y);
	pstm_init(NULL, &R);
	u32 k = n->used * DIGIT_BIT;//pstm_count_bits(n);
	pstm_set(&Y, k);
	pstm_set(&X, 2);
	pstm_exptmod(NULL, &X, &Y, n, &R); //R = 2^k % n
	//pstm_set(&Y, 1);
